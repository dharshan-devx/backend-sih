...
from .beneficiary_json import beneficiary_to_json
...
#api/views.py (same file)

def beneficiary_full_data_by_username(request, username):
    ...
    try:
        ...
        return HttpResponseNotFound("Beneficiary not found for this user")

    data = beneficiary_to_json(beneficiary)
    return JsonResponse(data, safe=False)

@login_required
def beneficiary_full_data_current_user(request):
    ...
    ben = Beneficiary.objects.filter(user=request.user).first()
    if not ben:
        return JsonResponse({"detail": "Beneficiary not found for this user"}, status=404)

    data = beneficiary_to_json(ben)
...
ML_ENDPOINT = "http://127.0.0.1:5000/predict"  # change to your ML URL


def call_ml_model(payload_dict):
    
    try:
        resp = requests.post(ML_ENDPOINT, json=payload_dict, timeout=5)
...
@login_required
def beneficiary_create_or_update(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # If you have CaseDetails / extra forms, save them here too.

            # 👉 Build big JSON for ML
            payload = beneficiary_to_json(ben)

            # 👉 Call ML model (optional)
            ml_result = call_ml_model(payload)

            # Example: show result on a template
            return render(
                request,
                "beneficiary/summary.html",
                {"beneficiary": ben, "payload": payload, "ml_result": ml_result},
            )
    else:
        form = BeneficiaryEditForm(instance=beneficiary)

    return render(request, "beneficiary/form.html", {"form": form})




# api/beneficiary_ml_payload.py  (recommended new file)
from .models import Beneficiary

def build_ml_payload(beneficiary: Beneficiary) -> dict:
    b = beneficiary
    data: dict = {
        "beneficiary": {
            "id": str(b.id),  # "BEN100000"
            "user_id": b.user_id,
            ...
            "govt_subsidy_received": b.govt_subsidy_received,
            "aadhaar_verified": b.aadhaar_verified,
            "pan_available": b.pan_available,
            "bank_account_active": b.bank_account_active,

            "income_est": b.income_est,
            "estimated_monthly_income": b.estimated_monthly_income,
            "income_category": b.income_category,
            "employment_type": b.employment_type,
            "work_consistency_days": b.work_consistency_days,

            # CREDIT HISTORY INPUTS
            "number_of_loans": b.number_of_loans,
            "emi_due_delays": b.emi_due_delays,
            "credit_card_available": b.credit_card_available,
            "cibil_score": b.cibil_score,

            "case_type": b.case_type,
            "loans_dues_change_reason": b.loans_dues_change_reason,
    ...
    case = getattr(b, "case_details", None)
    if case:
        data["case_details"] = {
            ...
            # Bills / utilities
            "electricity_units": case.electricity_units,
            "electricity_bill": float(case.electricity_bill) if case.electricity_bill is not None else None,
            "payments_regularity": case.payments_regularity,
            "average_mobile_bill": float(case.average_mobile_bill) if case.average_mobile_bill is not None else None,
            "gas_bill": float(case.gas_bill) if case.gas_bill is not None else None,
            "gas_frequency": case.gas_frequency,

            # Employment / work
            "employment_type": case.employment_type,
            "working_days_per_month": case.working_days_per_month,
            "digital_payment_frequency": case.digital_payment_frequency,

            # Banking & financials
            "average_bank_balance": float(case.average_bank_balance) if case.average_bank_balance is not None else None,
            "cash_inflow": float(case.cash_inflow) if case.cash_inflow is not None else None,
            "cash_outflow": float(case.cash_outflow) if case.cash_outflow is not None else None,
            "transactions_per_month": case.transactions_per_month,
            "last_6_months_avg_bank_balance": float(case.last_6_months_avg_bank_balance)
                if case.last_6_months_avg_bank_balance is not None else None,
            "number_of_active_loans": case.number_of_active_loans,
            "total_properties_value": float(case.total_properties_value) if case.total_properties_value is not None else None,
            "wealth_index": float(case.wealth_index) if case.wealth_index is not None else None,
            "any_business": case.any_business,
            "insurance_coverage": case.insurance_coverage,
            "luxury_expenditures": float(case.luxury_expenditures) if case.luxury_expenditures is not None else None,
            "outstanding_bank_balance": float(case.outstanding_bank_balance)
                if case.outstanding_bank_balance is not None else None,
            "loan_purpose": case.loan_purpose,
            "loan_history_cibil": case.loan_history_cibil,
            "reasons_for_delay": case.reasons_for_delay,
            "total_emi_per_month": float(case.total_emi_per_month) if case.total_emi_per_month is not None else None,
            "number_of_due_delays": case.number_of_due_delays,

            "created_at": case.created_at.isoformat() if case.created_at else None,
        }

    # ---------- Loan history (inputs about past loans) ----------
    data["loan_history"] = []
    for lh in b.loans.all():
        data["loan_history"].append(
            {
                "amount": lh.amount,
                "tenure": lh.tenure,
                "repayment_status": lh.repayment_status,
                "created_at": lh.created_at.isoformat() if lh.created_at else None,
            }
        )

    # ---------- Loan applications (what they are asking for) ----------
    #data["loan_applications"] = []
    #for la in b.loan_applications.all():
    #    data["loan_applications"].append(
    #       {
    #           "loan_amount": float(la.loan_amount),
...
@login_required
@require_http_methods(["GET"])
def beneficiary_ml_input_by_phone(request, phone):
    profile = getattr(request.user, "profile", None)
    if not profile or profile.role != "beneficiary":
        return HttpResponseForbidden("Beneficiary access required.")

    ben = get_object_or_404(Beneficiary, phone=phone)

    if ben.user != request.user:
        return HttpResponseForbidden("You are not allowed to access this data.")

    payload = build_ml_payload(ben)
